...
This uses a small lexical scanner, not a full JavaScript parser or a replacement for the TypeScript compiler. It separates ordinary quoted strings, comments, and template text from code, and skips regex literals in common expression positions. Regex-versus-division ambiguity, JSX text, nested templates, and unusual syntax still need a real parser. Template interpolation is skipped along with template text, so imports inside it are missed. Escaped string specifiers and computed imports are not resolved. It does not track scope, so a locally shadowed `require` can be counted. Aliases, `tsconfig` paths, package exports, and bundler resolution are unsupported. Type-only imports count as edges. Bare imports (including packages and Node built-ins) are listed as external and are not checked for installation. Relative references outside the chosen root or to non-source assets are reported unresolved.
